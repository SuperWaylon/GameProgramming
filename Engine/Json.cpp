...
namespace nu::json
{
    ...
    {
        // check if the value has the "<name>" and is an array of size 2
        ...
        const rapidjson::Value& v = value[name.c_str()];

        // check that both elements are numbers
        if (!v[0].IsNumber() || !v[1].IsNumber())
        {
            std::cerr << "Could not read JSON value (Vector2):" << name << std::endl;
            return false;
        }
    ...
    {
        // check if the value has the "<name>" and is an array of size 3
